...
class Users extends Model
{
	...
	const TABLE_BANNED  = 'users_banned';   //banned users table
	const TABLE_ROLES   = 'user_roles';   //banned users table
	...
	function create_user($data, $activated = TRUE)
	{
		...
		$data['user_role'] = 1; //default user role
	 ...
	 *
	 * @param	int	 
	 * @return	obj if banned, else FALSE
	 */
	function is_banned($user_id)
	{		
		$query = $this->db->get_where(self::TABLE_BANNED, array('id' => $user_id));			
	...
	{		
		$data = array(
					'id' => $user_id,
					'ban_reason' => $reason,
					'ban_date' => gmdate("Y-m-d H:i:s", time())
					);
					
		$this->db->insert(self::TABLE_BANNED, $data);
	}
	...
	{			
		$this->db->delete(self::TABLE_BANNED, array('id' => $user_id));
	}
