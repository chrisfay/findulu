...
class Tank_auth
{
		...
		if ((strlen($login) > 0) AND (strlen($password) > 0)) {
			...
			if (!is_null($user = $this->ci->users->$get_user_func($login))) {	// login ok
				...
				if ($hasher->CheckPassword($password, $user->password)) {		// password ok
					...
					if ($bannedUser = $this->ci->users->is_banned($user->id)) {									// fail - banned
						$this->error = array('banned' => $bannedUser->ban_reason);
	...
	//check if user logged in is an activated admin
	function is_admin()
	{
		return $this->ci->session->userdata('user_role') === '64' && $this->ci->session->userdata('status') === STATUS_ACTIVATED; //
	}
